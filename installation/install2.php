...
<?php
if (!$DBcreated){
	if (!isset($DBhostname) || !$DBuserName || !$DBname) {
...
function db_err($step, $alert) {
	...
	echo "<body><form name=\"$step\" method=\"post\" action=\"install1.php\">
	...
	echo "<script>alert('".$alert."'); document.".$step.".submit(); </script></body></html>";
	exit();
}
